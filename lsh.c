...
void
EvaluateCommando(Pgm *current_pgm)
{
    int nmb_args = 0;
...
void
Execute_if_exist (char *const prog[]) { //TBI
    //printf("prog[0] = %s\n", Get_path(prog[0]));
...
int Run_pipe2(Pgm *first_pgm) 
{
    ...
    if (nmb_pipes == 0) {
        ...
        char *ce = &(com_exit[0]);
        if (strcmp(*(first_pgm->pgmlist), ce) == 0) {
        ...
        if (pid == 0) { //Doesn't work if arg is NULL, check for nmb of args
            Pgm *current_pgm = first_pgm;     

            EvaluateCommando(current_pgm);
        }
        wait(NULL);  
    }
    else 
